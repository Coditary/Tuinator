...
TUINATOR_TEST(nerd_icon_nested_paths_use_domain_prefixes) {
    TUINATOR_CHECK(
        std::strcmp(tuinator::weather_icon_path(tuinator::WeatherIcon::DaySunny), "weather-day-sunny") == 0);
    TUINATOR_CHECK(std::strcmp(tuinator::clock_icon_path(tuinator::ClockIcon::Time1), "clock-time-1") == 0);
    TUINATOR_CHECK(
        std::strcmp(tuinator::moon_icon_path(tuinator::MoonIcon::MoonFull), "moon-full") == 0);
    TUINATOR_CHECK(
        std::strcmp(tuinator::compass_icon_path(tuinator::CompassIcon::DirectionUp), "compass-direction-up")
        == 0);
    TUINATOR_CHECK(
        std::strcmp(tuinator::weather_icon_path(tuinator::WeatherIcon::WindEast), "weather-wind-east") == 0);
    TUINATOR_CHECK(
        std::strcmp(tuinator::measurement_icon_path(tuinator::MeasurementIcon::Celsius), "measurement-celsius")
        == 0);
    TUINATOR_CHECK(std::strcmp(tuinator::action_icon_path(tuinator::ActionIcon::Refresh), "action-refresh") == 0);
    TUINATOR_CHECK(
        std::strcmp(tuinator::astronomy_icon_path(tuinator::AstronomyIcon::Horizon), "astronomy-horizon") == 0);
}
