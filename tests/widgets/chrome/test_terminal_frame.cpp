...
TUINATOR_TEST(terminal_frame_renders_title_border_and_status_line) {
    ...
    style.content_background = tuinator::style_fg_bg(
        tuinator::Rgb{0xc0, 0xca, 0xf5},
        tuinator::Rgb{0x1a, 0x1b, 0x26});
    style.status_line.background = tuinator::style_fg_bg(
        tuinator::Rgb{0xa9, 0xb1, 0xd6},
        tuinator::Rgb{0x16, 0x16, 0x1e});
...
TUINATOR_TEST(terminal_frame_without_status_line_is_shorter) {
    ...
    tuinator::TerminalFrame frame(
        "No status",
        std::make_unique<tuinator::Label>("content"),
        options);
