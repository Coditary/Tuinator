...
class ThrobberRoot : public tuinator::VBox {
  public:
    explicit ThrobberRoot(tuinator::BoxOptions options) : tuinator::VBox(options) {}
...
int main() {
    ...
    content->add_child(std::make_unique<tuinator::Label>(
        "Press q to exit. Default throbber (random-looking step) plus named sets.", theme.heading));
    content->add_child(std::make_unique<tuinator::Label>("Throbbers / loading spinners", theme.muted));
    ...
    content->add_child(std::make_unique<tuinator::Label>("q to quit", theme.muted));
