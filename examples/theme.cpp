...
std::unique_ptr<tuinator::Widget> build_glyph_sample(const tuinator::Theme& theme, const char* title,
                                                     const char* body) {
...
int main() {
    ...
    glyph_row->add_child(build_glyph_sample(tuinator::dark_theme({.glyphs = tuinator::GlyphSet::Ascii}), "ASCII",
                                            "Fallback + - | borders"));
    glyph_row->add_child(build_glyph_sample(tuinator::dark_theme({.glyphs = tuinator::GlyphSet::Unicode}), "Unicode",
                                            "Box drawing: corners and lines"));
    glyph_row->add_child(build_glyph_sample(tuinator::dark_theme({.glyphs = tuinator::GlyphSet::UnicodeRounded}),
                                            "Rounded", "Softer corners where supported"));
    ...
    root->add_child(std::make_unique<tuinator::Label>(std::string("Auto-detected glyphs: ") + detected_name +
