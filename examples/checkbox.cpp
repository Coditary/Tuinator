...
public:
    explicit CheckboxRoot(tuinator::BoxOptions options)
...
void add_checkbox(
    ...
    auto checkbox = std::make_unique<tuinator::Checkbox>(
        ...
        [status, label](bool value) {
            status->set_text(std::string(label) + (value ? " enabled" : " disabled"));
        },
        theme);
...
int main() {
    ...
    content->add_child(std::make_unique<tuinator::Label>(
        "A customizable checkbox widget for Tuinator",
        theme.muted));
    ...
    auto custom_box = std::make_unique<tuinator::Checkbox>(
        "Fully custom glyphs + colors",
        true,
        custom,
        [status_ptr](bool value) {
            status_ptr->set_text(std::string("Fully custom glyphs + colors")
    ...
    root->add_child(std::make_unique<tuinator::Label>(
        "Up/Down Navigate  |  Space Toggle  |  Tab Next  |  q Quit",
        theme.muted));
