...
int main() {
    ...
    root->add_child(std::make_unique<tuinator::Label>(
        "Counter", tuinator::Style{.foreground = tuinator::Color::Cyan, .bold = true}));

    auto count_label = std::make_unique<tuinator::Label>(
        "Value: 0", tuinator::Style{.foreground = tuinator::Color::White, .bold = true});
    auto* count_label_ptr = count_label.get();
    root->add_child(std::move(count_label));

    auto update = [count_label_ptr, &count]() { count_label_ptr->set_text("Value: " + std::to_string(count)); };
    ...
    root->add_child(std::make_unique<tuinator::Label>(
        "Click buttons or use Tab | q to quit", tuinator::Style{.foreground = tuinator::Color::Yellow, .dim = true}));
