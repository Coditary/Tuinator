...
Grid::Grid(GridOptions options)
    : columns_(std::max(1, options.columns)),
      gap_(options.gap),
...
void Grid::layout(Rect bounds) {
    ...
    const int column_width = columns_ > 0
        ? std::max(0, (content_width - gap_ * (columns_ - 1)) / columns_)
        : 0;
    const int row_height = rows > 0
        ? std::max(0, (content_height - gap_ * (rows - 1)) / rows)
        : 0;
