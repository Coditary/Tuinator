...
void paint_vertical_scrollbar(
    Canvas& canvas,
    int x,
    int y,
    int height,
    const ScrollbarThumb& thumb,
    const ScrollbarGlyphs& glyphs,
    const ScrollbarPalette& palette,
    bool show_arrows) {
    if (height <= 0) {
        return;
    }
    ...
    if (!show_arrows) {
        for (int row = 0; row < height; ++row) {
            ...
            draw_glyph(
                canvas,
                {x, y + row},
                is_thumb ? glyphs.thumb : glyphs.vertical_track,
...
void paint_horizontal_scrollbar(
    Canvas& canvas,
    int x,
    int y,
    int width,
    ...
    if (!show_arrows) {
        for (int col = 0; col < width; ++col) {
            ...
            draw_glyph(
                canvas,
                {x + col, y},
                is_thumb ? glyphs.thumb : glyphs.horizontal_track,
                is_thumb ? palette.thumb : palette.track);
        }
        return;
    }
...
ScrollbarBehavior behavior_for_preset(ScrollbarPreset preset) {
    switch (preset) {
    case ScrollbarPreset::Minimal:
        return ScrollbarBehavior::minimal();
    default:
        return ScrollbarBehavior::classic();
    }
}

ScrollbarStyle style_for_preset(ScrollbarPreset preset) {
    switch (preset) {
    case ScrollbarPreset::Ascii:
        return ScrollbarStyle::ascii();
    case ScrollbarPreset::Minimal:
        return ScrollbarStyle::minimal();
    case ScrollbarPreset::Bold:
        return ScrollbarStyle::bold();
    case ScrollbarPreset::Thin:
...
ScrollbarOptions ScrollbarOptions::classic() {
    return {};
}

ScrollbarOptions ScrollbarOptions::from_parts(
...
void paint_scrollbars(Canvas& canvas, const ScrollbarOptions& options, const ScrollbarLayout& layout) {
    ...
    if (layout.metrics.show_vertical) {
        paint_vertical_scrollbar(
            canvas,
            vertical_x,
            layout.vertical_bar_y,
            layout.vertical_bar_height,
            layout.vertical_thumb,
            glyphs,
            palette,
            show_arrows);
    }
    ...
    if (layout.metrics.show_horizontal) {
        paint_horizontal_scrollbar(
            canvas,
            layout.horizontal_bar_x,
            horizontal_y,
            layout.horizontal_bar_width,
            layout.horizontal_thumb,
            glyphs,
            palette,
            show_arrows);
    }
...
void paint_scrollbars(
    ...
    int content_width,
    int content_height) {
    const ScrollbarLayout layout = compute_scrollbar_layout(
        canvas.size().width,
        canvas.size().height,
        content_width,
        content_height,
