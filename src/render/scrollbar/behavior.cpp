...
ScrollbarBehavior ScrollbarBehavior::classic() { return {}; }
...
ScrollbarBehavior ScrollbarBehavior::from_kind(ScrollbarBehaviorKind kind) {
    switch (kind) {
    case ScrollbarBehaviorKind::Minimal: return minimal();
    case ScrollbarBehaviorKind::Overlay: return overlay();
...
int vertical_track_offset(bool show_arrows) { return show_arrows ? 1 : 0; }

int vertical_track_length(int bar_height, bool show_arrows) { return std::max(0, bar_height - (show_arrows ? 2 : 0)); }

int horizontal_track_length(int bar_width, bool show_arrows) { return std::max(0, bar_width - (show_arrows ? 2 : 0)); }
...
bool handle_scrollbar_mouse(const MouseEvent& mouse, Point local, const ScrollbarLayout& layout,
    ...
    if (state.drag_mode != ScrollbarInteractionState::DragMode::None) {
        ...
        if (mouse.action == MouseAction::Move || mouse.action == MouseAction::Press) {
            if (state.drag_mode == ScrollbarInteractionState::DragMode::VerticalThumb) {
                ...
                const int next_y = scroll_y_for_vertical_thumb(layout, track_y, content_height,
                                                               layout.metrics.viewport_height, behavior.show_arrows);
            ...
            if (state.drag_mode == ScrollbarInteractionState::DragMode::HorizontalThumb) {
                const int track_x = local.x - layout.horizontal_bar_x - vertical_track_offset(behavior.show_arrows) -
                                    state.drag_thumb_offset;
                const int next_x = scroll_x_for_horizontal_thumb(layout, track_x, content_width,
                                                                 layout.metrics.viewport_width, behavior.show_arrows);
    ...
    switch (hit) {
    case ScrollbarHit::VerticalArrowUp: actions.scroll_by(0, -behavior.arrow_step); return true;
    case ScrollbarHit::VerticalArrowDown: actions.scroll_by(0, behavior.arrow_step); return true;
    case ScrollbarHit::HorizontalArrowLeft: actions.scroll_by(-behavior.arrow_step, 0); return true;
    case ScrollbarHit::HorizontalArrowRight: actions.scroll_by(behavior.arrow_step, 0); return true;
    case ScrollbarHit::VerticalThumb:
        if (behavior.allow_thumb_drag) {
            ...
            state.drag_thumb_offset = local.y - layout.vertical_bar_y - vertical_track_offset(behavior.show_arrows) -
    ...
    case ScrollbarHit::HorizontalThumb:
        if (behavior.allow_thumb_drag) {
            ...
            state.drag_thumb_offset = local.x - layout.horizontal_bar_x - vertical_track_offset(behavior.show_arrows) -
    ...
    case ScrollbarHit::VerticalTrack:
        if (behavior.allow_track_jump) {
            ...
            const int next_y = scroll_y_for_vertical_thumb(layout, centered, content_height,
                                                           layout.metrics.viewport_height, behavior.show_arrows);
            actions.scroll_to(scroll_x, next_y);
            return true;
        }
    ...
    case ScrollbarHit::HorizontalTrack:
        if (behavior.allow_track_jump) {
            ...
            const int next_x = scroll_x_for_horizontal_thumb(layout, centered, content_width,
                                                             layout.metrics.viewport_width, behavior.show_arrows);
            actions.scroll_to(next_x, scroll_y);
            return true;
        }
...
bool handle_scrollbar_wheel(const MouseEvent& mouse, const ScrollbarBehavior& behavior,
    ...
    switch (mouse.action) {
    case MouseAction::WheelUp: actions.scroll_by(0, -behavior.wheel_step); return true;
    case MouseAction::WheelDown: actions.scroll_by(0, behavior.wheel_step); return true;
    case MouseAction::WheelLeft: actions.scroll_by(-behavior.wheel_step, 0); return true;
    case MouseAction::WheelRight: actions.scroll_by(behavior.wheel_step, 0); return true;
    default: return false;
    }
}
