...
std::vector<CatalogRow> catalog_rows_for(NerdIconSet set) {
    ...
    case NerdIconSet::Moon: return rows_for_set<MoonIcon>(moon_icon_descriptor, moon_icon_glyph, all_moon_icons);
    case NerdIconSet::Clock: return rows_for_set<ClockIcon>(clock_icon_descriptor, clock_icon_glyph, all_clock_icons);
    ...
    case NerdIconSet::Measurement:
        return rows_for_set<MeasurementIcon>(measurement_icon_descriptor, measurement_icon_glyph,
                                             all_measurement_icons);
    case NerdIconSet::Astronomy:
        return rows_for_set<AstronomyIcon>(astronomy_icon_descriptor, astronomy_icon_glyph, all_astronomy_icons);
    case NerdIconSet::AirQuality:
        return rows_for_set<AirQualityIcon>(air_quality_icon_descriptor, air_quality_icon_glyph, all_air_quality_icons);
    ...
    case NerdIconSet::MarineAlert:
        return rows_for_set<MarineAlertIcon>(marine_alert_icon_descriptor, marine_alert_icon_glyph,
                                             all_marine_alert_icons);
...
int row_count_for(NerdIconSet set) { return static_cast<int>(catalog_rows_for(set).size()); }
...
void NerdIconCatalogGallery::paint(PaintContext& ctx) const {
    ...
    for (NerdIconSet set : {
        ...
        char header[96];
        std::snprintf(header, sizeof(header), "[[ %s ]] (%zu)", nerd_icon_set_label(set), rows.size());
